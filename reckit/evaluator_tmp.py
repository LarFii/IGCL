...
class Evaluator(object):
    ...
    * **Second**, this class and its evaluation metrics can automatically fit
    ...
    def evaluate_test(self, model, test_users=None):
        """Evaluate `model`.
        ...
        ia_id_5 = np.load('/mnt/zirui/code/GCNAE/dataset/amazon-book/amazon-book_5.npy')
        ia_id_15 = np.load('/mnt/zirui/code/GCNAE/dataset/amazon-book/amazon-book_15.npy')
        ia_id_80 = np.load('/mnt/zirui/code/GCNAE/dataset/amazon-book/amazon-book_80.npy')

        for batch_users in test_users:        
            # test_items = [self.user_pos_test[u] for u in batch_users]          

            result_5 = self._evaluate(ia_id_5, batch_users, model)
            result_15 = self._evaluate(ia_id_15, batch_users, model)
            result_80 = self._evaluate(ia_id_80, batch_users, model)
               
            batch_result_5.append(result_5)
            batch_result_15.append(result_15)
            batch_result_80.append(result_80)

        # concatenate the batch results to a matrix

        self._print(batch_result_5)
        self._print(batch_result_15)
        self._print(batch_result_80)
        exit()
        
        buf = '\t'.join([("%.8f" % x).ljust(12) for x in final_result])
        return final_result, buf

    def _evaluate(self, ia_id, batch_users, model):  
        items = []

        for u in batch_users:
            tmp = []
            n = -1
            for item in self.user_pos_test[u]:
                # tmp.append(item)
                if item in ia_id:
                    tmp.append(item)
                else:
        ...
        result = eval_score_matrix(ranking_score, items, self.metrics,
                                    top_k=self.max_top, thread_num=self.num_thread)  # (B,k*metric_num)

        return result
    
    def _print(self, batch_result):
        all_user_result = np.concatenate(batch_result, axis=0)  # (num_users, metrics_num*max_top)
        final_result = np.mean(all_user_result, axis=0)# (1, metrics_num*max_top)
